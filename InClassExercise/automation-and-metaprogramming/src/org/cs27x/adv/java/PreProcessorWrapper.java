...
public class PreProcessorWrapper implements MethodInterceptor {
	...
	/**
	 * 
	 * Update this method to wrap the passed in target object so that its method
	 * annotation logic will be executed on method calls. You should make the
	 * default InvocationProcessorFactor an instance of your new implementation
	 * created in step 1.
	 * 
	 */
	public static <Type> Type wrap(Type target) {
		return wrap(target, new InvocationProcessorFactoryImpl());
	}

	@SuppressWarnings("unchecked")
	public static <Type> Type wrap(Type target,
			InvocationProcessorFactory factory) {
	 ...
	 * 
	 *             You should probably make sure that processors isn't null...
	 * 
	 */
	public List<InvocationProcessor<?>> getProcessors(Method method) {
		List<InvocationProcessor<?>> processors = new LinkedList<InvocationProcessor<?>>();
		for (Annotation anno : method.getAnnotations()) {	
			if(this.factory_.getProcessor(anno)!=null){
				InvocationProcessor<Annotation> pro = this.factory_.getProcessor(anno);
				pro.setAnnotation(anno);
				processors.add(pro);		
			}
		}
		return processors;
